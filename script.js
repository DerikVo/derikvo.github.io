document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadStandardsData() {
    try {
      ...
      console.log("Standards data loaded successfully");
      return data;
    } catch (error) {
  ...
  // Only load standards data if we're on a page that needs it
  if (window.location.pathname.includes("content-standards-matcher.html")) {
    loadStandardsData().then((data) => {
      if (data) {
        window.standardsData = data;
        console.log("Standards data ready to use:", data);

        // Initialize any standards-related functionality here
        initializeStandardsFeatures(data);
      }
    });
  }

  function initializeStandardsFeatures(standardsData) {
    // Add your standards matching functionality here
    const searchInput = document.getElementById("standards-search");
    const resultsContainer = document.getElementById("standards-results");
    ...
    if (searchInput && resultsContainer) {
      searchInput.addEventListener("input", function (e) {
        ...
        const filteredResults = standardsData.filter((item) => {
          return (
  ...
  function displayResults(results, container) {
    if (results.length === 0) {
      container.innerHTML = "<p>No matching standards found.</p>";
      return;
    }
